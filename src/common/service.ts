...
export abstract class BaseService<T, GenericCreateDto, GenericUpdateDto> {
  ...
  async findAll(paginationDto: PaginationDto): Promise<T[]> {
    try {
      ...
      const queryBuilder: SelectQueryBuilder<T> = this.repository.createQueryBuilder(this.repository.metadata.tableName);
    ...
    try {
      ...
      switch (operator) {
        ...
        case 'notLike':
          ...
          break;
        case 'in':
          queryBuilder.andWhere(`${this.repository.metadata.tableName}.${field} IN (${value.map((str) => `'${str}'`).join(', ')})`);
          break;
        case 'between':
          if (Array.isArray(value)) {
            if (typeof value[0] === 'string') {
              queryBuilder.andWhere(`${this.repository.metadata.tableName}.${field} BETWEEN '${value[0]}' AND '${value[1]}'`);
              break;
            }
            if (typeof value[0] === 'number')
              queryBuilder.andWhere(`${this.repository.metadata.tableName}.${field} BETWEEN ${Math.min(...value)} AND ${Math.max(...value)}`);
            break;
          }
          break;
        default:
